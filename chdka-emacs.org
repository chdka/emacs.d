Sdeft#+title: My Literate Emacs Configuration
...
** FEATURE Rename large =txt= file with to other extension         :feature:
When using deft, it scans all files defined in =deft-extensions=.
I'll like to include =txt= files also, due to large exports on my work system the hog and do not
give knowledge, only information.

- [ ] rename =txt= files to =tsv= (tab separated files)  or =csv= (comma separated files)
...
#+RESULTS:
: chdka-emacs--env-emacs-server-file
...
#+begin_src emacs-lisp
(setq org-agenda-files (list (expand-file-name (concat chdka-emacs--env-emacs-home-org "/GED/GED.org"))
                             (expand-file-name (concat chdka-emacs--env-emacs-home-org "/GED/refile.org"))
                             (expand-file-name (concat chdka-emacs--env-emacs-home-org "/GED/GED-work.org"))))
#+end_src

#+RESULTS:
| d:/christian/Documents/MyOrg/GED/GED.org | d:/christian/Documents/MyOrg/GED/refile.org | d:/christian/Documents/MyOrg/GED/GED-work.org |

***** Work related:  Outlook 2 Orgmode                       :@behaviour:
...
(when (not chdka-emacs--const-is-own-device)
   ...
   (add-to-list 'org-agenda-files (expand-file-name (concat chdka-emacs--env-emacs-home-org "/_ol2orgmode/")))
  )
#+end_src
...
(setq org-capture-templates
      (quote (("t" "todo" entry (file+headline
                                 (lambda () (concat chdka-emacs--env-emacs-home-org "/GED/GED.org")) "Niet Tijdgebonden")
              ...
              ("m" "meeting" entry (file+headline
                                    (lambda () (concat chdka-emacs--env-emacs-home-org "/GED/GED.org")) "Tijdgebonden")
               "* >>> %?\n %^t") ; <-- add to my list of meetings.
              
              ; https://github.com/sprig/org-capture-extension
              ("p" "protocol" entry (file+headline
                                     (lambda () (concat chdka-emacs--env-emacs-home-org "/GED/GED.org")) "Refile")
...
#+RESULTS:
| t | todo              | entry | (file+headline (lambda nil (concat chdka-emacs--env-emacs-home-org /GED/GED.org)) Niet Tijdgebonden) | * .T. %? |
| i | interrupt, urgent | entry | (file+headline (lambda nil (concat chdka-emacs--env-emacs-home-org /GED/GED.org)) Niet Tijdgebonden) | * .I. %? |
| n | next              | entry | (file+headline (lambda nil (concat chdka-emacs--env-emacs-home-org /GED/GED.org)) Niet Tijdgebonden) | * .N. %? |
| m | meeting           | entry | (file+headline (lambda nil (concat chdka-emacs--env-emacs-home-org /GED/GED.org)) Tijdgebonden)      | * >>> %? |
...
#+begin_src emacs-lisp
  (use-package deft
    ...
    :config (if chdka-emacs--const-is-own-device
                ; it is my own device
                (setq deft-directory  (expand-file-name "ChriszunNotities" chdka-emacs--env-emacs-home-org)
                      deft-extensions '("md" "txt" "org")
                      deft-default-extension "md") ; workaround, the default extension is not set properly
              ; it is my work device
              (setq deft-directory  (expand-file-name "_orgroam" chdka-emacs--env-emacs-home-org)
